...
----
	Flux<org.tanglongan.model.User> users  = client.get().retrieve().bodyToFlux(org.tanglongan.model.User.class)
----
...
----
	val users = client.get().retrieve().bodyToFlux<org.tanglongan.model.User>()
	// or (both are equivalent)
	val users : Flux<org.tanglongan.model.User> = client.get().retrieve().bodyToFlux()
----
...
* toString() of the form "org.tanglongan.model.User(name=John, age=42)"
